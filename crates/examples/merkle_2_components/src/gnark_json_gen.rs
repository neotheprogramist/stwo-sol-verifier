...
pub fn convert_stark_proof(
    ...
    let sampled_values: Vec<Vec<Vec<QM31Def>>> = proof
        .sampled_values
        .iter()
        .map(|tree| {
            tree.iter()
                .map(|col| col.iter().map(|val| QM31Def::from(*val)).collect())
                .collect()
        })
        .collect();
    ...
    let queried_values: Vec<Vec<u32>> = proof
        .queried_values
        .iter()
        .map(|tree| tree.iter().map(|val| val.0).collect())
        .collect();

    let decommitments: Vec<MerkleDecommitmentDef> = proof
        .decommitments
        .iter()
        .map(|d| MerkleDecommitmentDef {
    ...
    let inner_layers: Vec<FriLayerProofDef> = proof
        .fri_proof
        .inner_layers
        .iter()
        .map(|l| FriLayerProofDef {
...
pub fn convert_verification_params(
    components: Vec<&dyn Component>,
    components_log_sizes: Vec<u32>,
    n_preprocessed_columns: usize,
    mask_offsets: Vec<TreeVec<Vec<Vec<isize>>>>,
    components_claimed_sum: Vec<SecureField>,
    components_preprocessed_columns: Vec<Vec<PreProcessedColumnId>>,
    ...
    for (i, comp) in components.iter().enumerate() {
        let info = ComponentInfoDef {
            ...
            log_size: components_log_sizes[i],
            mask_offsets: mask_offsets[i]
                .0
                .iter()
                .map(|tree| {
                    tree.iter()
                        .map(|col| col.iter().map(|&off| off as i32).collect())
                        .collect()
                })
                .collect(),
        ...
        component_params.push(ComponentParamsDef {
            ...
            claimed_sum: QM31Def::from(components_claimed_sum[i]),
            info,
        });
    }
    ...
    for i in 0..8 {
        let start = i * 4;
        let bytes = [
            channel_digest[start],
            channel_digest[start + 1],
            channel_digest[start + 2],
            channel_digest[start + 3],
        ];
