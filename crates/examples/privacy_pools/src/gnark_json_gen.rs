...
// Helper struct to pass component information to convert_verification_params
...
pub fn convert_verification_params(
    ...
    for comp_info in component_infos {
        let info = ComponentInfoDef {
            max_constraint_log_degree_bound: comp_info.max_constraint_log_degree_bound,
            log_size: comp_info.log_size,
            mask_offsets: comp_info.mask_offsets.clone(),
            preprocessed_columns: comp_info.preprocessed_columns.clone(),
        };

        component_params.push(ComponentParamsDef {
            log_size: info.log_size,
            claimed_sum: QM31Def::from(comp_info.claimed_sum),
            info,
        });
    }

    let comps_struct = Components {
        components: components_dyn.to_vec(),
        n_preprocessed_columns,
    };
    let composition_log_degree_bound = comps_struct.composition_log_degree_bound();

    // Calculate extended log sizes from the first component's trace_log_degree_bounds
    let extended_log_sizes: Vec<Vec<u32>> = component_infos[0]
